...
import (
	...
	"github.com/cordum-io/cordum-packs/packs/servicenow/internal/config"
	"github.com/cordum-io/cordum-packs/packs/servicenow/internal/gatewayclient"
	"github.com/cordum-io/cordum-packs/packs/servicenow/internal/servicenowapi"
)

const (
	topicRead  = "job.servicenow.read"
	topicWrite = "job.servicenow.write"
)
...
var actionSpecs = map[string]actionSpec{
	"incidents.list":   {Name: "incidents.list", Topic: topicRead, Method: http.MethodGet, Path: "/api/now/table/incident"},
	"incidents.get":    {Name: "incidents.get", Topic: topicRead, Method: http.MethodGet, Path: "/api/now/table/incident/{sys_id}", PathParam: "sys_id", RequiredKeys: []string{"sys_id"}},
	"changes.list":     {Name: "changes.list", Topic: topicRead, Method: http.MethodGet, Path: "/api/now/table/change_request"},
	"changes.get":      {Name: "changes.get", Topic: topicRead, Method: http.MethodGet, Path: "/api/now/table/change_request/{sys_id}", PathParam: "sys_id", RequiredKeys: []string{"sys_id"}},
	"cmdb.list":        {Name: "cmdb.list", Topic: topicRead, Method: http.MethodGet, Path: "/api/now/table/cmdb_ci"},
	"cmdb.get":         {Name: "cmdb.get", Topic: topicRead, Method: http.MethodGet, Path: "/api/now/table/cmdb_ci/{sys_id}", PathParam: "sys_id", RequiredKeys: []string{"sys_id"}},
	"incidents.create": {Name: "incidents.create", Topic: topicWrite, Method: http.MethodPost, Path: "/api/now/table/incident", Body: true},
	"incidents.update": {Name: "incidents.update", Topic: topicWrite, Method: http.MethodPatch, Path: "/api/now/table/incident/{sys_id}", PathParam: "sys_id", RequiredKeys: []string{"sys_id"}, Body: true},
	"changes.create":   {Name: "changes.create", Topic: topicWrite, Method: http.MethodPost, Path: "/api/now/table/change_request", Body: true},
	"changes.update":   {Name: "changes.update", Topic: topicWrite, Method: http.MethodPatch, Path: "/api/now/table/change_request/{sys_id}", PathParam: "sys_id", RequiredKeys: []string{"sys_id"}, Body: true},
}
...
func New(cfg config.Config) (*Worker, error) {
	...
	worker, err := runtime.NewWorker(runtime.Config{
		...
		Capabilities:    []string{"servicenow"},
		Labels:          map[string]string{"adapter": "servicenow"},
		Type:            "servicenow",
	})
	if err != nil {
		return nil, err
...
func (w *Worker) handleJob(ctx context.Context, req *agentv1.JobRequest) (*agentv1.JobResult, error) {
	...
	client := servicenowapi.NewClient(profile.BaseURL, servicenowapi.Options{
		Token:     resolveSecret(profile.Token, profile.TokenEnv),
		Username:  resolveSecret(profile.Username, profile.UsernameEnv),
		Password:  resolveSecret(profile.Password, profile.PasswordEnv),
...
func (w *Worker) execute(ctx context.Context, client *servicenowapi.Client, spec actionSpec, params map[string]any) (any, int, error) {
	path := spec.Path
	if spec.PathParam != "" {
