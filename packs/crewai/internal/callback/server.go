...
// ProgressRequest captures a crew/task progress update from the Python sidecar.
...
func NewCallbackServer(cfg CallbackConfig) (*CallbackServer, error) {
	...
	mux.HandleFunc("/progress", server.handleProgress)
...
func (s *CallbackServer) handleProgress(w http.ResponseWriter, r *http.Request) {
	...
	if s.natsConn == nil || s.senderID == "" {
		writeJSONResponse(w, http.StatusServiceUnavailable, map[string]any{"ok": false, "error": "progress publishing unavailable"})
		return
	}

	var request ProgressRequest
	if err := decodeProgressRequest(r, &request); err != nil {
	...
	if request.ActiveAgent != "" {
		prefix := "active agent: " + strings.TrimSpace(request.ActiveAgent)
		if message != "" {
	...
	if request.TotalTasks > 0 {
		suffix := fmt.Sprintf(" (%d/%d tasks)", request.CompletedTasks, request.TotalTasks)
		if message != "" {
			message += suffix
		} else {
	...
	progress := &agentv1.JobProgress{
		...
		Percent: clampPercent(request.Percent),
		Message: strings.TrimSpace(message),
		Status:  agentv1.JobStatus_JOB_STATUS_RUNNING,
	}
	if err := runtime.PublishProgress(s.natsConn, progress, strings.TrimSpace(request.TraceID), s.senderID, nil); err != nil {
...
func decodeProgressRequest(r *http.Request, out *ProgressRequest) error {
	if strings.TrimSpace(r.Header.Get("Content-Type")) != "" && !strings.Contains(r.Header.Get("Content-Type"), "application/json") {
		return fmt.Errorf("content-type must be application/json")
	}
	decoder := json.NewDecoder(io.LimitReader(r.Body, 1<<20))
...
func clampPercent(value int32) int32 {
	if value < 0 {
		return 0
	}
	if value > 100 {
