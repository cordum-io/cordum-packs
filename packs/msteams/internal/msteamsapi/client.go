...
func (e *APIError) Error() string {
	return fmt.Sprintf("msteams api error (%d): %s", e.StatusCode, e.Message)
}
...
func NewClient(baseURL, token string, opts Options) *Client {
	...
	if userAgent == "" {
		userAgent = "cordum-msteams-worker"
	}
...
func messageFromValue(value any) string {
	...
	case map[string]any:
		if msg := messageFromValue(typed["message"]); msg != "" {
			if code := messageFromValue(typed["code"]); code != "" {
...
func requestIDFromHeaders(header http.Header) string {
	if val := strings.TrimSpace(header.Get("request-id")); val != "" {
		return val
	}
	if val := strings.TrimSpace(header.Get("client-request-id")); val != "" {
		return val
	}
	if val := strings.TrimSpace(header.Get("x-ms-request-id")); val != "" {
		return val
	}
	return ""
}
