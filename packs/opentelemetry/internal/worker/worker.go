...
func New(cfg config.Config) (*Worker, error) {
	...
	workerID := runtime.ResolveWorkerID("", "opentelemetry")
...
func (w *Worker) Run(ctx context.Context) error {
	...
	if len(subjects) == 0 {
		subjects = []string{"job.opentelemetry.*"}
	}
...
func (w *Worker) handleJob(ctx runtime.Context, payload map[string]any) (callResult, error) {
	...
	if ctx.Job.GetTopic() != "job.opentelemetry.read" {
		return callResult{}, fmt.Errorf("read actions must use job.opentelemetry.read topic")
	}
